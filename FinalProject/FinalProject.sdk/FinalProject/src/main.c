...
int main(void)
{
	...
	while(1)
	{
		while(value == 0) //wait for data to be sent to the UART module
		{
			value = XGpio_DiscreteRead(&uart_rx_data_present, 1);
			setChannelData(); //Continuously update the PWM modules
		}
		while(value == 1)
		{
			delay();
			XGpio_DiscreteWrite(&uart_rx, 2, 1);	// Set read pin to send data to the PS
			XGpio_DiscreteWrite(&uart_rx, 2, 0);
			inputBuffer[i] = XGpio_DiscreteRead(&uart_rx, 1); // Read data from UART
			i++;
			value = XGpio_DiscreteRead(&uart_rx_data_present, 1);
		}

		if(inputBuffer[0] == E) //Enable Channels
		{
			enableChannel(&LEDInst, inputBuffer);
		}
		if(inputBuffer[0] == D)	//Disable Channels
		{
			disableChannel(&LEDInst, inputBuffer);
		}
		if(inputBuffer[0] == F) //Set channel frequencies
		{
			setChannelFrequency(inputBuffer);
		}
		if(inputBuffer[0] == P) //Set channel baud rates
		{
